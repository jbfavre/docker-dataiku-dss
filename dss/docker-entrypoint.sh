...
then
    ...
    echo "** Installing Dataiku as Design Node with community license"
    DSSINSTALLOPT="-p 10000"
elif [ ${IS_DSS_CONFIGURED} -eq 0 -a ${HAS_DSS_LICENSE} -eq 1 ];
then
    echo "* DSS is not installed"
    echo "* Custom license file found"
    echo "** Installing Dataiku as Design Node with custom license"
    DSSINSTALLOPT="-l ${DATAIKULICENSE} -p 10000"
elif [ ${IS_DSS_CONFIGURED} -eq 1 ];
then
    echo "* DSS hass already been installed"
    echo "** Upgrading Dataiku Design Node"
    DSSINSTALLOPT="-u -y"
else
    echo "*** Oooops… Shouldn't be there :-/"
    exit 1
fi
/bin/su - dataiku -c "cd ${DATAIKUPROG} && ./installer.sh -d ${DATAIKUDATA} ${DSSINSTALLOPT}"
